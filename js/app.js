...
(function () {
  ...
  function handleEditMessage(ordinal) {
    ...
    for (var i = 0; i < msgs.length; i++) {
      if (msgs[i].role === 'user' && typeof msgs[i].content === 'string') {
        count++;
        if (count === ordinal) { target = i; break; }
      }
    }
    if (target < 0) return;
    var text = msgs[target].content;
    session.messages = msgs.slice(0, target);   // remove it + all later turns
    Store.setCurrentSession(session);
    UI.renderSession(session);
    UI.fillComposer(text);
  }
  ...
  function handleNewChat() {
    ...
    if (hasContent && !window.confirm('Clear this conversation? It will NOT be saved to your coach summary.')) return;
    var fresh = Store.startNewSession();
    UI.renderSession(fresh);
    UI.focusInput();
    UI.toast('Started a new conversation.', 'info');
  }
  ...
  function boot() {
    ...
    UI.init({
      ...
      onEditMessage: handleEditMessage, onNewChat: handleNewChat
    });
    UI.showScreen('coach');
