...
(function () {
  ...
  /* Lets the coach start an on-screen countdown timer for timed holds / stretches /
   * rest periods (e.g. a 30s plank or a 45s hamstring stretch). */
  var START_TIMER_TOOL = {
    name: 'start_timer',
    description: 'Start an on-screen countdown timer for the user. Use it whenever an exercise or stretch ' +
      'is held or timed (planks, wall sits, Copenhagen plank, stretches, carries by time) or for a rest ' +
      'period — e.g. when you tell them to hold something for N seconds. The app shows the timer and ' +
      'chimes when done; in a voice call you will also be told when it finishes.',
    input_schema: {
      type: 'object',
      properties: {
        seconds: { type: 'number', description: 'Duration in seconds (e.g. 30).' },
        label: { type: 'string', description: 'Short label, e.g. "Plank hold" or "Hamstring stretch".' }
      },
      required: ['seconds']
    }
  };
  ...
  function buildSystemPrompt(opts) {
    ...
    var s = 'You are an upbeat, knowledgeable personal fitness coach and workout logger. ' +
      ...
      'with structured data. This INCLUDES stretches, mobility and flexibility work (cat-cow, hip-flexor ' +
      'lunge, hamstring stretch, etc.) — log those with type "flexibility" and a duration_min when held ' +
      'for time. Turn natural language into sets/reps/weight, durations and distances. ' +
      ...
      '1b) TIMER: When something is held or timed (a plank, wall sit, a stretch you tell them to hold for ' +
      'N seconds, a timed carry, or a rest period), call start_timer so a countdown appears on screen.\n' +
      ...
      'VIDEOS: The app AUTOMATICALLY shows a tappable "▶ Watch" demo-video button under your message ' +
      'for any exercise you name. So you can ALWAYS help with videos — never say you cannot share or ' +
      'show videos. When the user asks for a video, just name the exercise and tell them to tap the ' +
      '"▶ Watch" button below your message (it appears in chat; in a voice call they can find it on the ' +
      'Coach screen or Exercises tab). Do not paste raw URLs.';
    ...
    function step() {
      ...
      return streamRequest(body, { onText: handlers.onText, signal: opts.signal }).then(function (res) {
        ...
        res.content.forEach(function (block) {
          ...
          } else if (block.name === 'start_timer') {
            var ti = block.input || {};
            var secs = Math.max(1, Math.min(3600, Math.round(ti.seconds || 0)));
            if (handlers.onTimer) handlers.onTimer(secs, (ti.label || '').trim());
            resultText = JSON.stringify({ ok: secs > 0, seconds: secs });
          } else {
